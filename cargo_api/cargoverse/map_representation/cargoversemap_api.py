...
class CargoverseMap:
    ...
    def __init__(self) -> None:
        ...
        self.city_name_to_city_id_dict = {"PIT": PITTSBURGH_ID, 
                                          "MIA": MIAMI_ID, 
                                          "T01":T01_ID,
                                          "T02":T02_ID,
                                          "T03":T03_ID,
...
#             if city_name in CARLA_TOWNS:
#                 ############## TO DO: ADD BELOW FEATURES TO CARLA TOWNS  #############
#                 continue
...
#             print(city_name, ' is ready')
    ...
    def get_lane_segment_centerline_2d(self, lane_segment_id: int, city_name: str) -> np.ndarray:
        """
        We return a 3D centerline for any particular lane segment.
...
#             lane_centerline = self.append_height_to_2d_city_pt_cloud(lane_centerline, city_name)
    ...
    def get_lane_segment_polygon_2d(self, lane_segment_id: int, city_name: str) -> np.ndarray:
        """
        Just for Cargoverse API. 
        do not need ground height and se2 files for computing laneGCN model.
        
        Hallucinate a 2d lane polygon based around the centerline.  
        ...
        Returns:
            lane_polygon: Array of polygon boundary (K,2), with identical and last boundary points
        """
        lane_centerline = self.city_lane_centerlines_dict[city_name][lane_segment_id].centerline
        lane_polygon = centerline_to_polygon(lane_centerline[:, :2])
        return lane_polygon
        ...
        if dist > threshold:
            ...
            if child_lanes is not None:
                for child in child_lanes:
                    centerline = self.get_lane_segment_centerline_2d(child, city_name)
    ...
    def get_lane_segments_containing_xy(self, query_x: float, query_y: float, city_name: str) -> List[int]:
        ...
        if neighborhood_lane_ids is not None:
            for lane_id in neighborhood_lane_ids:
                lane_polygon = self.get_lane_segment_polygon_2d(lane_id, city_name)
