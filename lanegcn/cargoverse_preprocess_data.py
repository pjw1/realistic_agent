...
from cargoverse_data import ArgoDataset as Dataset, from_numpy, ref_copy, collate_fn
...
def main():
    ...
    config["val_workers"] = 0  #original 32
    config["workers"] = 0 
    config['cross_dist'] = 6
...
def modify(config, data_loader, save):
    ...
    for i, data in enumerate(data_loader):
        ...
        for j in range(len(data)):
            ...
            out.append(preprocess(to_long(gpu(data[j])), config['cross_dist'], data=data[j]))
