...
class ArgoDataset(Dataset):
    ...
    def read_argo_data(self, idx):
        ...
        # Cargo dataset doen't have AGENT obj type
    ...
    def get_lane_graph(self, data):
        ...
        for lane_id in lane_ids:
            ...
            if x.max() < x_min or x.min() > x_max or y.max() < y_min or y.min() > y_max:
                ...
                polygon = self.am.get_lane_segment_polygon_2d(lane_id, data['city'])
...
class ArgoTestDataset(ArgoDataset):
    def __init__(self, split, config, train=False):
        ...
        self.avl = ArgoverseForecastingLoader(avl_cargo)  # only for get 'argo_id'
        if 'preprocess' in config and config['preprocess']:
            if train:
                self.split = np.load(split, allow_pickle=True)
            else:
#                 self.split = np.load(split, allow_pickle=True)
                self.split = np.load(split_cargo, allow_pickle=True)
                print('preprocessed cargo_test has loaded')
        else:
